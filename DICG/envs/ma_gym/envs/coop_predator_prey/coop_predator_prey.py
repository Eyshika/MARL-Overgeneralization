...
class CoopPredatorPrey(gym.Env):
    """
    ...
    def __init_full_obs(self):
        ...
        for prey_i in range(self.n_preys):
            while True:
                pos = [random.randint(0, self._grid_shape[0] - 1), random.randint(0, self._grid_shape[1] - 1)]
                if self._is_cell_vacant(pos) and (self._neighbour_agents(pos, self._full_obs)[0] == 0):
    ...
    def _neighbour_agents(self, pos, full_obs):
        # check if agent is in neighbour
        _count = 0
        neighbours_xy = []
        if self.is_valid([pos[0] + 1, pos[1]]) and PRE_IDS['agent'] in full_obs[pos[0] + 1][pos[1]]:
            _count += 1
            neighbours_xy.append([pos[0] + 1, pos[1]])
        if self.is_valid([pos[0] - 1, pos[1]]) and PRE_IDS['agent'] in full_obs[pos[0] - 1][pos[1]]:
            _count += 1
            neighbours_xy.append([pos[0] - 1, pos[1]])
        if self.is_valid([pos[0], pos[1] + 1]) and PRE_IDS['agent'] in full_obs[pos[0]][pos[1] + 1]:
            _count += 1
            neighbours_xy.append([pos[0], pos[1] + 1])
        if self.is_valid([pos[0], pos[1] - 1]) and PRE_IDS['agent'] in full_obs[pos[0]][pos[1] - 1]:
            neighbours_xy.append([pos[0], pos[1] - 1])
        ...
        agent_id = []
        for x, y in neighbours_xy:
            agent_id.append(int(full_obs[x][y].split(PRE_IDS['agent'])[1]) - 1)
        return _count, agent_id
    
    def getHeuristicCooperativeness(self, previous_full_obs):
        cooperativeness = [True for _ in range(self.n_agents)]
        for prey_i in range(self.n_preys):
            if self._prey_alive[prey_i]:
                predator_neighbour_count, agent_ids = self._neighbour_agents(self.prey_pos[prey_i], self._full_obs)
                if predator_neighbour_count == 1:
                    '''
                    single predator approached the prey
                    check if someone else is no cooperative
                    
                    check last frame, if any other agents appears
                    two steps away from this prey (so they could catch it).
                    '''
                    un_cooperative_agent = []
                    prey_neighbor_delta = [[1,0], [-1,0], [0, 1], [0,-1]]
                    for delta in prey_neighbor_delta:
                        prey_neighbor_pose = [self.prey_pos[prey_i][0] + delta[0], self.prey_pos[prey_i][1] + delta[1]]
                        if self.is_valid(prey_neighbor_pose) and \
                           (PRE_IDS['agent'] not in self._full_obs[prey_neighbor_pose[0]][prey_neighbor_pose[1]]):
                            predator_neighbour_count2, agent_ids2 = self._neighbour_agents(prey_neighbor_pose, previous_full_obs)
                            if (predator_neighbour_count2 == 1 and agent_ids[0] not in agent_ids2) or predator_neighbour_count2 > 1:
                                # print("there are some uncooperative boys for agent"+str(agent_ids[0]), agent_ids2)
                                cooperativeness[agent_ids[0]] = False
                                break
    ...
    def step(self, agents_action):
        ...
        previous_full_obs = copy.deepcopy(self._full_obs)

        for agent_i, action in enumerate(agents_action):
            if not (self._agent_dones[agent_i]):
                self.__update_agent_pos(agent_i, action)

        coop = self.getHeuristicCooperativeness(previous_full_obs)
        for prey_i in range(self.n_preys):
            if self._prey_alive[prey_i]:
                predator_neighbour_count, n_i = self._neighbour_agents(self.prey_pos[prey_i], self._full_obs)
                ...
                if self._prey_alive[prey_i]:
                    ...
                    for _ in range(5):
                        ...
                        if self._neighbour_agents(self.__next_pos(self.prey_pos[prey_i], _move), self._full_obs)[0] == 0:
                            prey_move = _move
                            break
        ...
        return self.get_agent_obs(), rewards, self._agent_dones, {'prey_alive': self._prey_alive, 'coop': coop}
