...
class CentralizedMAPPOWithAuxLoss(MABatchPolopt):
    """Centralized Multi-agent PPO with an auxiliary binary classifiers (cooperativeness prediction).
    ...
    def train_once(self, itr, paths):
        ...
        obs, avail_actions, actions, rewards, valids, baselines, returns, coops = \
        ...
        with torch.no_grad():
            loss_before = self._compute_loss(itr, obs, avail_actions, actions, 
                                             rewards, valids, baselines,coops)
        ...
        for mini_epoch in range(self._optimization_mini_epochs):
            for start in range(0, len(rewards), step_size):
                ...
                loss = self._compute_loss(itr, obs[ids], avail_actions[ids], 
                                          ...
                                          valids[ids], baselines[ids], coops[ids])
    ...
    def _compute_loss(self, itr, obs, avail_actions, actions, rewards, valids, 
        ...
        policyLoss = -torch.cat(valid_objectives).mean()
    ...
    def process_samples(self, itr, paths):
        ...
        # print("obs shape: ", path['observations'].shape)
        ...
        cooperatives = torch.stack([
            pad_one_to_last(path['coops'],
                        total_length=self.max_path_length,
                        axis=0) for path in paths
        ])
        print('coop:', cooperatives.shape)
        ...
        return obs, avail_actions, actions, rewards, valids, baselines, returns, cooperatives