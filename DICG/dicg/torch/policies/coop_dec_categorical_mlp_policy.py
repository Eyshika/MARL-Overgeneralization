...
class CoopDecCategoricalMLPPolicy(Policy, MultiHeadedMLPModule):
    """CategoricalMLPPolicy
    ...
    def __init__(self,
        ...
        MultiHeadedMLPModule.__init__(self,
                                        n_heads=2,
                                        input_dim=self._obs_dim,
                                        output_dims=[self._action_dim, 1],
    ...
    def forward(self, obs, avail_actions):
        ...
        policyOutputs, CoopOuput = super().forward(obs)
        # print("output shape: ", policyOutputs.shape)
        dist = Categorical(logits=policyOutputs) # the last ouput is the cooperativeness prediction
        ...
        # output the cooperativeness
        cooperativeness = torch.sigmoid(CoopOuput)
        return masked_dist, cooperativeness
    ...
    def get_actions(self, observations, avail_actions, greedy=False):
        ...
        with torch.no_grad():
            ...
            dist, _ = self.forward(torch.Tensor(observations), torch.Tensor(avail_actions))
